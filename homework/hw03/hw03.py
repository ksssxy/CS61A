...
def balanced(m):
    ...
    if is_planet(m):
        ...
        left_arm = length(left(m)) * total_weight(end(left(m)))
        right_arm = length(right(m)) * total_weight(end(right(m)))
        if left_arm != right_arm:
            return False
        elif not balanced(end(left(m))) or not balanced(end(right(m))):
            return False
        else:
            return True
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert lower_bound(y) * upper_bound(y) > 0, 'interval y spans zero'
...
def quadratic(x, a, b, c):
    ...
    if lower_bound(x) <= point <= upper_bound(x):
        point_value = a*point*point + b*point + c
        lower = min(point_value, left, right)
        upper = max(point_value, left, right)
    else:

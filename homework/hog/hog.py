...
def swine_align(player_score, opponent_score):
    ...
    if player_score > 0 and opponent_score > 0:
        if math.gcd(player_score, opponent_score) >= 10:
            return True
        else:
            return False
    else:
    ...
    while score0 < goal and score1 < goal:
        ...
        if who == 1:
            ...
            if extra_turn(score0, score1) == False:
                who = other(who)
        say = say(score0, score1) ###需使用赋值符号进行调用同一函数
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if cur - last_score > running_high:
            cur_high = cur - last_score
            print(f"{cur_high} point(s)! The most yet for Player {who}")
        return announce_highest(who, cur, cur_high)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    num, max_roll_num= 0, 0
    for i in range(10):
        averaged_dice = make_averaged(roll_dice, trials_count)
        averaged_result = averaged_dice(i + 1, dice)
        if averaged_result > max_roll_num:
            max_roll_num = averaged_result
            num = i + 1
    return num
    # END PROBLEM 9
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    return 0 if extra_turn(score + free_bacon(opponent_score), opponent_score) or bacon_strategy(score, opponent_score, cutoff, num_rolls) == 0 else num_rolls# Replace this statement
    # END PROBLEM 11
